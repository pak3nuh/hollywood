...
Once a colleague of mine told me that "almost every problem in software can be solved with code generation and an additional abstraction layer", and I find this to be accurate.

By generating code we get two pros:
1. Fast execution and lower memory footprint
2. Actually get to see what the code is doing (fully debuggable)

The developer experience is also improved because first a prototype is built by hand, then, once it works, it is generated automatically.
...
The main difference between generating kotlin source or java source is that with kotlin we need to place the output in the directory given by `processingEnv.options["kapt.kotlin.generated"]` and we don't have multiple rounds of annotation processing.
