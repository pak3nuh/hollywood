...
class GenerationContext(val logger: Logger, private val types: Types, private val elements: Elements) {
    ...
    private val kotlinConversionMap = mapOf<String, String>(
            java.util.List::class.java.canonicalName!! to List::class.qualifiedName!!
    )
    ...
    fun asKotlinTypeName(javaType: TypeElement): ClassName {
        val javaTypeName = javaType.asTypeName().toString()
        val kotlinTypeName = kotlinConversionMap[javaTypeName] ?: javaTypeName
        return ClassName.bestGuess(kotlinTypeName)
    }
